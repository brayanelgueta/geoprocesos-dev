...
const Widget = (props) => {
  ...
  const changeGeoProcess = useSelector(
  ...
  const ejecutarConsulta = async () => {
    ...
    try {
      ...
      if (imagen1 == null && imagen2 == null) {
        throw new Error(
          `Error en la carga de imagenes: ${imagen1} , ${imagen2}`
        );
      }
  ...
  const loadImageryLayer = async (url_file) => {
    try {
      ...
      const [ImageryTileLayer] = await loadModules([
    ...
    try {
      ...
      if (jimuMapView && imageryLayer) {
        const loadEsriModules = async () => {
          ...
          if (
            !isNaN(selectedImageries[0].OBJECTID) &&
            ...
            if (results.features.length === 2) {
              const geometries = results.features.map(
                (feature) => feature.geometry
              );

              // Validar si hay intersección entre las dos geometrías
              const intersectan = geometryEngine.intersects(
                geometries[0],
                geometries[1]
              );
              ...
              console.error(
                "No se encontraron ambas geometrías para validar la superposición"
              );
            }
          } else {
            throw new Error(
              "Uno o ambos valores no se pueden convertir a entero."
            );
          }
        };
            ...
            {isEjecuted === true ? (
              ...
              <div className="main-content" id="btn1Data">
                <div className="contentNameImgs">
                  <p id="parrafoValidacion">
                    {t("validation")} :{" "}
                    <b id="validacionCD">{t("withoutValidation")}</b>
                  </p>

                  {selectedImageries?.map((item, index) => (
                    <div
                      id="contentNameImg"
                      className="contentNameImg colorBoderBottom1"
                    >
                      <p>
                        {t("image")} {index + 1}:
                      </p>
                ...
                <div className="contentBtnFlujo">
                  {!isValid ? (
                    loading === true ? (
                      ...
                      <Button onClick={validarSuperposicion} size="sm">
                        {t("validate")}
                      </Button>
                    )
                  ) : loadQuery === true ? (
                    ...
                    <Button onClick={ejecutarConsulta} size="sm" type="primary">
                      {t("runQuery")}
                    </Button>
                  )}
                </div>
