    ...
    switch (formula) {
      case "NDVI":
        ...
        if (selectedSensor) {
          ...
          if (sensor?.title === "Worldview2") {
            setSelectedFormula(
              `(B${sensor.bands[6]?.value} - B${sensor.bands[4]?.value}) / (B${sensor.bands[6]?.value} + B${sensor.bands[4]?.value})`
            );
          }
          if (sensor?.title === "OWD_FasatC_MS_v2_20250312") {
            setSelectedFormula(
              `(B${sensor.bands[3]?.value} - B${sensor.bands[0]?.value}) / (B${sensor.bands[3]?.value} + B${sensor.bands[0]?.value})`
            );
          }
          if (sensor?.title === "Default") {
            setSelectedFormula(
              `(B${sensor.bands[3]?.value} - B${sensor.bands[2]?.value}) / (B${sensor.bands[3]?.value} + B${sensor.bands[2]?.value})`
            );
          }
        }
      ...
      case "SAVI":
        ...
        if (selectedSensor) {
          ...
          if (sensor?.title === "Worldview2") {
            setSelectedFormula(
              `((B${sensor.bands[6]?.value} - B${sensor.bands[4]?.value}) / (B${sensor.bands[6]?.value} + B${sensor.bands[4]?.value} + 0,5)) * (1 + 0,5)`
            );
          }

          if (sensor?.title === "OWD_FasatC_MS_v2_20250312") {
            setSelectedFormula(
              `((B${sensor.bands[3]?.value} - B${sensor.bands[0]?.value}) / (B${sensor.bands[3]?.value} + B${sensor.bands[0]?.value} + 0,5)) * (1 + 0,5)`
            );
          }
          if (sensor?.title === "Default") {
            setSelectedFormula(
              `((B${sensor.bands[3]?.value} - B${sensor.bands[2]?.value}) / (B${sensor.bands[3]?.value} + B${sensor.bands[2]?.value} + 0,5)) * (1 + 0,5)`
            );
          }
        }
      ...
      case "NDWI":
        ...
        if (selectedSensor) {
          ...
          if (sensor?.title === "Worldview2") {
            setSelectedFormula(
              `(B${sensor.bands[2]?.value} - B${sensor.bands[6]?.value}) / (B${sensor.bands[2]?.value} + B${sensor.bands[6]?.value})`
            );
          }
          if (sensor?.title === "OWD_FasatC_MS_v2_20250312") {
            setSelectedFormula(
              `(B${sensor.bands[1]?.value} - B${sensor.bands[3]?.value}) / (B${sensor.bands[1]?.value} + B${sensor.bands[3]?.value})`
            );
          }
          if (sensor?.title === "Default") {
            setSelectedFormula(
    ...
    if (selectedImageries.length > 0) {
      ...
      if (img1Data !== null) {
        ...
        if (jimuMapView && esriModulesRef.current) {
          ...
          const { ImageryLayer, RasterFunction, RasterStretchRenderer } =
            ...
            <div className="contentSliceMask-Base">
              ...
              <div className="contentSliceMask">
                ...
                <div className="contentSliceMaskPri">
                  ...
                  <label className="label1Mask">
                    {(inputTypeAreaMin + inputTypeAreaMin).toFixed(4)}
                  </label>
