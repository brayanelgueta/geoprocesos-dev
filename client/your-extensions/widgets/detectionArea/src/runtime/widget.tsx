...
const Widget = (props) => {
  ...
  const capaTemporal = async (dagerZoneLayer, riesgo: String) => {
    setShowBuffer(true);
    const [Sketch, GraphicsLayer, SimpleFillSymbol, geometryEngine] =
      await loadModules([
        "esri/widgets/Sketch",
    ...
    sketchEdit.on("update", async (event) => {
      // Aplicar el estilo cyan cada vez que se actualiza la selección
  ...
  const getLayerData = async () => {
    ...
    const [Polygon, FeatureLayer] = await loadModules([
      "esri/geometry/Polygon",
      "esri/layers/FeatureLayer",
    ]);
        ...
        if (response.features.length > 0) {
          ...
          const featuresFormateados = response.features.map((f) => {
            return {
              id:
                f.attributes?.numero ||
                f.attributes?.cod_compania ||
                f.attributes?.sistema ||
                f.attributes?.id ||
              ...
              name:
                f.attributes?.nombre ||
                f.attributes?.NOMBRE ||
                f.attributes?.nom_unidad ||
                f.attributes?.nom_com ||
                f.attributes?.nombre_ins ||
                f.attributes?.nom_estacion ||
                f.attributes?.compania ||
                f.attributes?.soporte ||
                f.attributes?.empresa ||
                f.attributes?.nom_obra ||
                `Código_${f.attributes?.OBJECTID}` ||
                "No aplica",
              latitud:
                f.attributes?.latitud ||
                f.attributes?.coor_y ||
                f.attributes?.coord_norte ||
                f.attributes?.COORD_NORT ||
                f.attributes?.Y ||
                f.attributes?.geometry?.y ||
                "No aplica",
              longitud:
                f.attributes?.longitud ||
                f.attributes?.coor_x ||
                f.attributes?.coord_este ||
                f.attributes?.COORD_ESTE ||
                f.attributes?.X ||
                f.attributes?.geometry?.x ||
                "No aplica",
            };
          });
        ...
        <Page size="A4" style={styles.page} wrap>
          ...
          <View style={styles.metricas}>
            ...
            <Text style={styles.text}>
              {t("reportMeters")} {areaBuffer.toFixed(2)}
            </Text>
            <Text style={styles.text}>
              {t("reportKm")} {(areaBuffer / 1000000).toFixed(2)}
            </Text>
            <Text style={styles.text}>
              {t("reportHa")} {(areaBuffer / 10000).toFixed(2)}
            </Text>
          </View>
              ...
              <>
                ...
                <div className="area-detection-buttons">
                  ...
                  {loadingInundacion === true ? (
                    ...
                    <Button onClick={() => startDA("inundacion")} size="sm">
                      {t("inundacion")}
                    </Button>
                  )}
                </div>
            ...
            {showBuffer && (
              <div id="btnDA2" className="area-detection-buffer">
                ...
                <div className="area-detection-buffer-actions">
                  <p>{t("bufferLabel")}</p>
                  <div className="area-detection-buffer-input">
                    <input
                      id="inputBuffer"
                      type="text"
                      min="1"
                  ...
                  {ready === true && (
                    <PDFDownloadLink
                      ...
                      fileName={`Reporte Zonificación ${date.toLocaleDateString(
                        "es-ES"
                      )} " "${date.toLocaleTimeString("es-ES")}.pdf`}
                    >
                      {({ loading }) =>
                        loading ? (
                          <Button size="sm" type="primary">
