...
const Widget = (props: AllWidgetProps<unknown>) => {
  ...
  const [inputTypeAreaMax] = useState(1);
  const [inputTypeAreaMin] = useState(-1);
  ...
  const loadEsriModules = async () => {
    try {
      const [RasterFunction, ImageryLayer, RasterStretchRenderer] =
        await loadModules([
  ...
  const createResultLayer = async () => {
    ...
    try {
      ...
      if (selectedImage !== null) {
        const { RasterFunction, RasterStretchRenderer } =
          esriModulesRef.current;
        ...
        const existingLayer = await jimuMapView.view.map.findLayerById(
          "Fórmula índices"
        );
        if (existingLayer) {
          jimuMapView.view.map.remove(existingLayer);
  ...
  useEffect(() => {
    if (jimuMapView) {
      const layer = jimuMapView.view.map.allLayers.find(
        (layer) => layer.title == "Fórmula índices"
      );
      if (layer) {
  ...
  useEffect(() => {
    if (selectedSensor) {
      const sensor =
        sensors.find((s) => s.title === selectedSensor.title) || sensors[2];
          ...
          <div className="bands-main-container">
                ...
                <div className="band-inputs">
                  <div>
                    ...
                    <Select
                      ...
                      size="sm"
                      onChange={(e) =>
                        setBands({ ...bands, r: e.target.value })
                      }
                    >
                  ...
                  <div>
                    ...
                    <Select
                      ...
                      size="sm"
                      onChange={(e) =>
                        setBands({ ...bands, g: e.target.value })
                      }
                    >
                      {availableBands.map((band, index) => (
            ...
            <div className="button-group">
              {loading === true ? (
                ...
                <Button onClick={createResultLayer} size="sm" type="primary">
                  {t("apply")}
                </Button>
              )}

              <Button onClick={resetRenderer} size="sm" type="primary">
                {t("reset")}
              </Button>
            </div>
          </div>
